...
public class InternovaSellersController {
    ...
    @SuppressWarnings({"Duplicates", "unchecked"})
    public Result getInternovaSellers(final Http.Request request) throws IOException, ExecutionException, InterruptedException {
        ObjectNode result = Json.newObject();
        JsonNode json = request.body().asJson();
        if (json == null) {
            return badRequest("Expecting Json data");
            ...
            if (json == null) {
                ...
                CompletableFuture<HashMap<String, Object>> getFuture = CompletableFuture.supplyAsync(() -> {
                            return jpaApi.withTransaction(
                                    entityManager -> {
                                        String bSql = "select * from internova_sellers where 1=1 ";
                                        HashMap<String, Object> returnList_future = new HashMap<String, Object>();
                                        List<HashMap<String, Object>> filalist = new ArrayList<HashMap<String, Object>>();
                                        List<InternovaSellersEntity> internovaSellersEntityList
                                                = (List<InternovaSellersEntity>) entityManager.createNativeQuery(
                                                bSql, InternovaSellersEntity.class).getResultList();
